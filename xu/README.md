...
對於想塞進一本書的分量來說，這本書所涵蓋的範圍是稍嫌貪心的。本書開發的是把C語言的程式碼轉（source code）換成組合語言（assembly code）用的程式，也就是所謂的C編譯器（compiler）。而這個編譯器本身也是以C語言所開發。現階段的目標是self-host－也就是自幹的編譯器要可以編譯其自己的程式碼。
